...
The `from` field is present in all peer-sent messages. It is absent only
in `ready` (generated by the transport adapter, which is not a peer) and
in `close` when sent by the adapter.
...
| `method`       | `req`                                                                                    | MUST be `"encrypted"`. The real method name is inside the encrypted `sealed` payload (§7.4). |
...
| `event`        | `evt`                                                                                    | MUST be `"encrypted"`. The real event name is inside the encrypted `sealed` payload (§7.4). |
...
signed), method and event names are **always encrypted**:

- The sender MUST set the `method` field to `"encrypted"` (for `req`) and
  the `event` field to `"encrypted"` (for `evt`). No other values are
...
- The receiver MUST read the real name from the `_method` / `_event`
  field in the decrypted payload. If the field is missing, reject with
  `invalid_params`.
- If a peer receives a message with `method` or `event` set to any value
  other than `"encrypted"`, it MUST reject with `protocol_error`.
...
- **Method/event name encryption (Section 7.4):** Mandatory. The
  `method` and `event` fields on the wire are always `"encrypted"`.
  Real names are carried inside the `sealed` payload. There is no
  opt-out mechanism.
